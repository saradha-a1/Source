...
export default async function () {
    ...
    try {
        ...
        // Select the first invited user from the list (you can modify this logic)
        const selectedUser = invitedUsers[0]; // You can choose a user based on some condition

        const username = selectedUser.Name; // Use the username from the API response
        const email = selectedUser.Email; // Use the email from the API response

        console.log(`Selected invited user: ${username} - ${email}`);
...
await page.waitForTimeout(5000);
        ...
        // Step 4: Click on the "View inbox" button
        ...
        await page.waitForTimeout(5000);
        // Step 5: Click on the email subject "Welcome to Kissflow!"
        ...
        console.log('Clicked on "Welcome to Kissflow!" email subject');

        // Step 6: Click on "Get started" link
        ...
        // Step 7: Type "User@123" in the first password field
        console.log('Waiting for first password field');
        await page.waitForSelector(
            '(//input[@id="newPassword"])[1]', 
            { timeout: 10000 }
        );
        await page.type(
            '(//input[@id="newPassword"])[1]', 
        ...
        // Step 8: Type "User@123" in the second password field
        console.log('Waiting for second password field');
        await page.waitForSelector(
            '(//input[@id="confirmPassword"])[1]', 
            { timeout: 10000 }
        );
        await page.type(
            '(//input[@id="confirmPassword"])[1]', 
        ...
        // Wait for a few seconds to ensure the actions are completed
